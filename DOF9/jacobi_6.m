...
J_UR=simplify(J)
simplify(det(J))
% -a2*a3*sin(th5)*((d5*cos(th2 + 2*th3 + th4))/2 - (d5*cos(th2 + th4))/2 - (a2*sin(th2 + th3))/2 + (a3*sin(th2))/2 + (a2*sin(th2 - th3))/2 - (a3*sin(th2 + 2*th3))/2)
% -a2*a3*sin(th5)*((d5*cos(th2 + 2*th3 + th4))/2 - (d5*cos(th2 + th4))/2 - (a2*sin(th2 + th3))/2 + (a3*sin(th2))/2 + (a2*sin(th2 - th3))/2 - (a3*sin(th2 + 2*th3))/2)
% --------------------------------------------------
T16=fkine_GP7(th,6);
for i=1:6 %速度雅可比矩阵；v w与th6无关；
...
%% 
J=[J1 J2 J3 J4 J5 J6].'; simplify(J) %输出其转置，方便word处理公式

% De=det(J); simplify(De) %虚拟六轴 -d7*sin(th5)*(a8 + d7*sin(th8))
% f=factor(De);
...
function Ji=jacob(A)
    ...
    % J6=[px*ny-py*nx px*oy-py*ox px*ay-py*ax nz oz az].';
end
...
function T = DH_inverse(theta,d,a,alpha) %DH_inverse()T_stand_inv()
